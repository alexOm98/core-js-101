...
function roundToPowerOfTen(num, pow) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (i < pow) resultArr.push('0');
    else resultArr.push(arr[i]);
  }
  resultArr = resultArr.reverse().map((el) => +el);
  if (+num.toString()[resultArr.length - pow] >= 5) {
    resultArr[resultArr.length - pow - 1] = resultArr[resultArr.length - pow - 1] + 1;
  }
